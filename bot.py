...
async def handle_otp(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not otp.isdigit() or len(otp) not in (5, 6):
        await update.message.reply_text("❌ Enter 5-6 digits.")
        return OTP
    ...
    if result.get("status") == "2fa_required":
        ...
        await update.message.reply_text(f"✅ Verified: `{result['user']}`", parse_mode="Markdown")
...
async def handle_password(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not phone or not otp:
        await update.message.reply_text("❌ Start over")
        return ConversationHandler.END
    ...
    if result.get("status") == "success":
        ...
        await update.message.reply_text(f"❌ Failed: {result.get('reason', 'Invalid password')}")
